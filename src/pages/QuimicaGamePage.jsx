...
const PERSONAJES = [
  ...
  { id: 7,  min: 960,  emoji: '👩‍🔬', nombre: 'Laboratorista Senior',  desc: 'Sabe cuándo la reacción va mal.',      color: '#0047AB', bg: '#F7F8FA' },
        ...
        <div className="relative h-2 rounded-full overflow-hidden" style={{ background: '#dbeafe' }}>
          <div className="h-full rounded-full transition-all duration-500 ease-out relative overflow-hidden"
            style={{
              ...
              background: 'linear-gradient(90deg,#0047AB,#2563C7,#8B8E92,#26282B)',
...
function PantallaFinal({ stats, onReiniciar }) {
  ...
  return (
    <div className="flex flex-col items-center text-center px-4 py-6 gap-4">
      ...
      <div>
        <p className="text-[10px] font-bold mb-3 tracking-widest" style={{ color: '#2563C7' }}>TU EVOLUCIÓN</p>
      ...
      {showCard && (
        <div className="w-full rounded-2xl overflow-hidden" style={{ background: '#fff', border: '1px solid #C2C5C8' }}>
          ...
          <div className="mx-3 mb-3 rounded-xl p-3" style={{ background: '#f8f9ff', border: '1px solid #C2C5C8' }}>
            <div className="grid grid-cols-5 gap-1">
              {PERSONAJES.map(p => (
                <div key={p.id} className="flex flex-col items-center p-1 rounded-lg transition-all"
                  ...
                  <span className="text-[7px] leading-tight text-center mt-0.5" style={{ color: puntos >= p.min ? p.color : '#8B8E92' }}>
                    {p.nombre.split(' ')[0]}
                  </span>
                </div>
              ))}
...
export default function QuimicaGamePage() {
  ...
  return (
    <div className="max-w-lg mx-auto" style={{ paddingBottom: 100 }}>
      <div className="rounded-2xl overflow-hidden shadow-sm mx-3 mt-3" style={{ background: '#ffffff', border: '1px solid #C2C5C8' }}>
