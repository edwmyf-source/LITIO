...
const CAT_LABELS = { productos:'Productos', servicios:'Servicios', empleos:'Empleos', informacion:'Información' }

const CAT_PLACEHOLDER = {
  productos:   'Escribe aquí lo que buscas o lo que tienes en productos...',
  servicios:   'Escribe aquí el servicio que ofreces o que necesitas...',
  empleos:     'Escribe aquí la vacante o el empleo que buscas...',
  informacion: 'Escribe aquí lo que quieres compartir con la comunidad...',
  default:     '¿Qué estás buscando o qué tienes para ofrecer?',
}
...
export default function InlinePublishBox({ onOpen, onPublished }) {
  ...
  // Detectar ubicación solo al expandir (una vez)
  const handleExpand = () => {
    setExpanded(true)
    if (!location && !detectingLoc) {
  ...
  const handleCollapse = () => {
    setExpanded(false)
    setForm({ category: '', subcategory: '', title: '', intent: 'ofrecen' })
    setFiles([])
  }
  ...
  const addFiles = (incoming) => {
    ...
    for (const f of Array.from(incoming).slice(0, remaining)) {
      if (f.size > MAX_SIZE_MB * 1024 * 1024) { toast(`${f.name} supera ${MAX_SIZE_MB} MB`, 'error'); continue }
      const kind = fileKind(f)
      const preview = kind === 'image' ? URL.createObjectURL(f) : null
      valid.push({ file: f, preview, kind })
    }
    ...
    try {
      const post = await createPost(
        { author_id: session.user.id, title: form.title.trim(), content: '',
          category: form.category, subcategory: form.subcategory || null, location: location || null, intent: form.intent || 'ofrecen' },
        files.map(f => f.file)
      )
      onPublished?.(post)
      handleCollapse()
  ...
  // ── COLLAPSED: celda simple ──────────────────────────────────────────────
  if (!expanded) {
    return (
      <div className="bg-white border border-ink-300 rounded-2xl p-3 mb-3">
        <div className="flex items-center gap-2.5">
          <UserAvatar
            seed={session?.user?.id || 'me'}
            avatarUrl={profile?.avatar_url || null}
            size={36}
          />
          <button
            onClick={handleExpand}
            className="flex-1 text-left px-4 py-2.5 rounded-full border border-ink-300 bg-ink-100/50 text-[13px] text-ink-400 hover:bg-slate-50 transition-colors"
          >
            {CAT_PLACEHOLDER.default}
          </button>
        </div>
        {/* Accesos rápidos — también expanden */}
        <div className="flex items-center gap-1 mt-2.5 ml-[44px]">
          <button onClick={handleExpand}
            className="flex items-center gap-1.5 text-[12px] text-ink-500 hover:text-ink-700 px-3 py-1 rounded-2xl hover:bg-slate-50 font-medium">
  ...
  // ── EXPANDED: formulario completo inline ─────────────────────────────────
  return (
    <div className="bg-white border border-ink-300 rounded-2xl p-3 mb-2.5">

      {/* Header */}
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <UserAvatar seed={session?.user?.id || 'me'} avatarUrl={profile?.avatar_url || null} size={32} />
          <div>
            <span className="font-medium text-sm text-ink-900">Nueva publicación</span>
            {!detectingLoc && location && (
              <p className="text-[10px] text-brand-600 font-medium flex items-center gap-0.5">
                <MapPin size={10} /> {location}
              </p>
            )}
            {detectingLoc && (
              <p className="text-[10px] text-ink-400">Detectando ubicación...</p>
            )}
          </div>
        </div>
        <button onClick={handleCollapse} aria-label="Cerrar" className="p-1 rounded hover:bg-slate-50 text-ink-500">
          <X size={16} />
        </button>
      </div>
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        <textarea
          ...
          rows={3}
          className="w-full px-3 py-2 rounded-2xl border border-ink-300 text-[13px] focus:outline-none focus:border-brand-600 bg-white resize-none"
          required
          maxLength={280}
        />
              ...
              return (
                <button key={c.value} type="button"
                  ...
                  <span className={`text-[10px] font-medium leading-tight ${
                    form.category === c.value ? 'text-brand-700' : 'text-ink-600'
                  }`}>
                    {CAT_LABELS[c.value]}
                  </span>
                </button>
        ...
        <div className="flex items-center justify-between pt-1 border-t border-ink-100">
          ...
          <div className="flex items-center gap-1.5">
            <button type="button" disabled={files.length >= MAX_FILES}
              ...
              style={{ width: 46, height: 46 }} title="Foto">
            ...
            <button type="button" disabled={files.length >= MAX_FILES}
              ...
              style={{ width: 46, height: 46 }} title="PDF">
