    ...
    <header className="sticky top-0 z-40 w-full"
      ...
      <div className="max-w-5xl mx-auto h-14 flex items-center gap-4 px-4">
        ...
        <button onClick={() => navigate('/feed')} className="flex items-center gap-2 flex-shrink-0">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: '#0d1b3e' }}>
            <FlaskConical size={16} color="#9fa8da" />
          </div>
          <span className="text-base font-bold hidden lg:block" style={{ color: '#0d1b3e', letterSpacing: '.5px' }}>Rodio</span>
        </button>

        {/* Buscador */}
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg flex-1 max-w-xs"
          style={{ background: '#f0f2f8', border: '0.5px solid #e8eaef' }}>
          <Search size={14} color="#9fa8da" />
          <span className="text-xs" style={{ color: '#9fa8da' }}>Buscar en Rodio...</span>
        </div>
          ...
          {navItems.map(({ id, label, Icon, badge }) => {
            ...
            return (
              <button key={id} onClick={() => navigate(id)}
                ...
                style={{ borderBottomColor: active ? '#0d1b3e' : 'transparent', minWidth: 56 }}>
                <div className="relative">
                  <Icon size={18} color={active ? '#0d1b3e' : '#9fa8da'} />
                  {badge > 0 && (
                ...
                <span className="text-[10px] font-medium" style={{ color: active ? '#0d1b3e' : '#9fa8da' }}>
                  {label}
                </span>
              </button>
            )
        ...
        {/* Divider */}
        <div className="w-px h-7 flex-shrink-0" style={{ background: '#e8eaef' }} />
        ...
        <div className="relative flex-shrink-0" ref={menuRef}>
          <button onClick={() => setMenuOpen(o => !o)}
            ...
            <span className="text-[10px] font-medium flex items-center gap-0.5" style={{ color: '#9fa8da' }}>
          ...
          {menuOpen && (
            <div className="absolute right-0 top-full mt-1 rounded-xl overflow-hidden z-50"
              ...
              <div className="px-4 py-3 border-b" style={{ borderColor: '#e8eaef' }}>
                <p className="text-sm font-bold" style={{ color: '#0d1b3e' }}>{name}</p>
                <p className="text-xs" style={{ color: '#9fa8da' }}>{session?.user?.email}</p>
                {profile?.quimica_personaje && (
                  <div className="flex items-center gap-1.5 mt-1.5 px-2 py-1 rounded-lg" style={{ background: '#f0f2f8' }}>
              ...
              {menuItems.map(({ label, Icon, path }) => (
                <button key={path} onClick={() => navigate(path)}
                  ...
                  style={{ color: '#0d1b3e' }}>
                  <Icon size={15} style={{ color: '#1a237e' }} />
                  {label}
                </button>
              ))}
